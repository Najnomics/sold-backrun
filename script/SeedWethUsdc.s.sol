...
/// @notice WETH mock paired with the existing bond asset (shown as USDC) so bids still donate.
contract SeedWethUsdcScript is Script {
    ...
    function run() public {
        ...
        address usd = SearcherBond(bonds).asset();

        vm.startBroadcast(pk);
        MockERC20 weth = new MockERC20("Wrapped Ether", "WETH", 18);
        MockERC20 usdc = MockERC20(usd);
        ...
        string memory s0 = wethIs0 ? "WETH" : "USDC";
        string memory s1 = wethIs0 ? "USDC" : "WETH";
        ...
        console2.log("token0", address(token0), s0);
        console2.log("token1", address(token1), s1);
    }
}
